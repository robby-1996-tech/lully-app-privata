...
def first_non_empty(form, name: str) -> str:
    """Quando ci sono campi duplicati con lo stesso name (es. Experience + All-inclusive),
    Flask può leggere il primo (anche vuoto). Qui prendiamo il primo valore NON vuoto."""
...
# -------------------------
# BOOKING (con pacchetti completi)
# -------------------------
...
def booking_new():
    ...
    if request.method == "POST":
        ...
        payload = {
            ...
            # compatibilità vecchia lista/dettaglio
        ...
        # -------------------------
        # ✅ VALIDAZIONI OPERATIVE PACCHETTI (BACKEND)
        # -------------------------
...
# -------------------------
# MODIFICA / ELIMINA / PDF
# -------------------------
def row_to_form_dict(row):
    """Trasforma una sqlite3.Row in un dict compatibile con i template (stringhe)."""
    d = {k: ("" if row[k] is None else str(row[k])) for k in row.keys()}
    # ricrea checkbox extra_*
    csv = (row["extra_keys_csv"] or "").strip()
    keys = [k for k in csv.split(",") if k.strip()] if csv else []
    for k in keys:
        d[f"extra_{k}"] = "on"
    # checkbox consensi
    try:
        if int(row["consenso_privacy"] or 0) == 1:
            d["consenso_privacy"] = "on"
        if int(row["consenso_foto"] or 0) == 1:
            d["consenso_foto"] = "on"
    except Exception:
        pass
    return d
...
@app.route("/prenotazioni/<int:booking_id>/edit", methods=["GET", "POST"])
def prenotazione_modifica(booking_id: int):
    if not is_logged_in():
        return redirect(url_for("login"))
    ...
    if request.method == "POST":
        ...
        # valori che possono arrivare duplicati
        torta_choice = first_non_empty(request.form, "torta_choice")
        torta_interna_choice = first_non_empty(request.form, "torta_interna_choice")
        torta_gusto_altro = first_non_empty(request.form, "torta_gusto_altro")
        catering_baby_choice = first_non_empty(request.form, "catering_baby_choice")
        dessert_bimbi_choice = first_non_empty(request.form, "dessert_bimbi_choice")
        dessert_adulti_choice = first_non_empty(request.form, "dessert_adulti_choice")
        ...
        payload = {
            ...
            "data_evento": (row["data_evento"] or ""),
            "madre_nome_cognome": (request.form.get("madre_nome_cognome") or "").strip(),
            "madre_telefono": (request.form.get("madre_telefono") or "").strip(),
            ...
            # consensi modificabili (firma invariata)
            "consenso_privacy": 1 if request.form.get("consenso_privacy") else 0,
            "consenso_foto": 1 if request.form.get("consenso_foto") else 0,
            ...
            "catering_baby_choice": catering_baby_choice,

            "dessert_bimbi_choice": dessert_bimbi_choice,
            "dessert_adulti_choice": dessert_adulti_choice,

            "torta_choice": torta_choice,
            "torta_interna_choice": torta_interna_choice,
            "torta_gusto_altro": torta_gusto_altro,
        ...
        conn.execute(
            ...
            ''',
            (
                payload["nome_festeggiato"],
                payload["eta_festeggiato"],
                payload["data_compleanno"],

                payload["madre_nome_cognome"],
                payload["madre_telefono"],
                payload["padre_nome_cognome"],
                payload["padre_telefono"],
                payload["indirizzo_residenza"],
                payload["email"],

                payload["invitati_bambini"],
                payload["invitati_adulti"],
                ...
                payload["consenso_foto"],

                payload["acconto_eur"],
                payload["pacchetto_personalizzato_dettagli"],
                payload["catering_baby_choice"],
                payload["dessert_bimbi_choice"],
                payload["dessert_adulti_choice"],
                payload["torta_choice"],
                payload["torta_interna_choice"],
                payload["torta_gusto_altro"],
                ",".join(payload["extra_keys"]),
                str(totals["totale"]),
                contract_text,
                booking_id,
            ),
        ...
        conn.commit()
        conn.close()
        return redirect(url_for("prenotazione_dettaglio", booking_id=booking_id))

    form = row_to_form_dict(row)
    conn.close()
    return render_template_string(
        EDIT_BOOKING_HTML,
        app_name=APP_NAME,
...
@app.route("/prenotazioni/<int:booking_id>/delete", methods=["POST"])
def prenotazione_elimina(booking_id: int):
    if not is_logged_in():
        return redirect(url_for("login"))
    conn = get_db()
...
@app.route("/prenotazioni/<int:booking_id>/pdf")
def prenotazione_pdf(booking_id: int):
    ...
    story = []
    story.append(Paragraph(f"Contratto / Riepilogo Prenotazione #{row['id']} – {APP_NAME}", styles["Title"]))
    story.append(Spacer(1, 12))

    when = row["event_date"] or row["data_evento"] or "-"
    slot = row["slot_code"] or "-"
    area = row["area"] or "-"
    story.append(Paragraph(f"<b>Data evento</b>: {when} &nbsp;&nbsp; <b>Slot</b>: {slot} &nbsp;&nbsp; <b>Area</b>: {area}", styles["Normal"]))
    story.append(Spacer(1, 8))
    story.append(Paragraph(f"<b>Festeggiato</b>: {row['nome_festeggiato'] or '-'} &nbsp;&nbsp; <b>Età</b>: {row['eta_festeggiato'] or '-'}", styles["Normal"]))
    story.append(Spacer(1, 8))
    story.append(Paragraph(f"<b>Invitati</b>: {(row['invitati_bambini'] or 0)} bimbi / {(row['invitati_adulti'] or 0)} adulti", styles["Normal"]))
    story.append(Spacer(1, 8))
    story.append(Paragraph(f"<b>Pacchetto</b>: {row['pacchetto'] or '-'}", styles["Normal"]))
    story.append(Spacer(1, 12))

    story.append(Paragraph("Dettagli pacchetto (contratto)", styles["Heading3"]))
    story.append(Spacer(1, 6))
    story.append(Preformatted(row["dettagli_contratto_text"] or "", styles["Code"]))
    story.append(Spacer(1, 12))

    story.append(Paragraph(f"<b>Acconto</b>: {row['acconto_eur'] or '-'}", styles["Normal"]))
    story.append(Spacer(1, 6))
    tot = str(row["totale_stimato_eur"] or "-")
    story.append(Paragraph(f"<b>Totale stimato</b>: EUR {tot.replace('.', ',')}", styles["Normal"]))
    story.append(Spacer(1, 12))
    ...
    if note:
        ...
        story.append(Spacer(1, 6))
        story.append(Preformatted(note, styles["Code"]))

    doc.build(story)
    buf.seek(0)
    return send_file(buf, as_attachment=True, download_name=f"prenotazione_{row['id']}.pdf", mimetype="application/pdf")
...
<head>
  ...
  <style>
    ...
    a.btn, button.btn {display:inline-block; padding:10px 12px; border-radius:10px; background:#0a84ff; color:#fff; text-decoration:none; font-weight:800; border:none; cursor:pointer;}
  ...
  <div class="card">
    ...
    <form method="post" id="bookingForm">
      ...
      <div class="section" id="experienceBox" style="display:none;">
        ...
        <div class="section">
          <h3>Servizi extra (seleziona uno o piu)</h3>
          <div class="row">
...
(function() {
  ...
  function refreshVisibility() {
    const p = pacchetto ? pacchetto.value : '';

    if (experienceBox) experienceBox.style.display = (p === 'Lullyland Experience') ? 'block' : 'none';
    if (allInclusiveBox) allInclusiveBox.style.display = (p === 'Lullyland all-inclusive') ? 'block' : 'none';
    if (personalizzatoBox) personalizzatoBox.style.display = (p === 'Personalizzato') ? 'flex' : 'none';
...
<body>
  <div class="card">
    ...
    <div style="display:flex; gap:10px; flex-wrap:wrap; margin:10px 0 14px;">
      <a class="btn" href="/prenotazioni/{{b['id']}}/edit">✏️ Modifica</a>
      <a class="btn" href="/prenotazioni/{{b['id']}}/pdf">📄 Download PDF</a>
      <form method="post" action="/prenotazioni/{{b['id']}}/delete" onsubmit="return confirm('Vuoi eliminare definitivamente questa prenotazione?');" style="margin:0;">
        <button class="btn" type="submit" style="background:#b00020;">🗑️ Elimina</button>
      </form>
    </div>
